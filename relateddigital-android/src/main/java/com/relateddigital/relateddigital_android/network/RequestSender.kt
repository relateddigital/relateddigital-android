        ...
        when (currentRequest.domain) {
            ...
            Domain.IN_APP_SPIN_TO_WIN_PROMO_CODE -> {
                val sInterface = SApiClient.getClient(model.getRequestTimeoutInSecond())
                        ?.create(ApiMethods::class.java)
                val call: Call<ResponseBody> = sInterface!!.getPromotionCodeRequestJsonResponse(currentRequest.headerMap, currentRequest.queryMap)
                        ...
                        try {
                            ...
                            if (rawJsonResponse != "") {
                                ...
                                if (jsonResponse.getBoolean("success") && !jsonResponse.getString("promocode").equals("")) {
                                    ...
                                    val visilabsResponse = VisilabsResponse(jsonResponse, null, null, null, null)
                                    ...
                                    val visilabsResponse = VisilabsResponse(null, null, "Empty promotion code - auth issue", null, "Empty promotion code - auth issue")
                                ...
                                val visilabsResponse = VisilabsResponse(null, null, "empty string", null, "empty string")
                                currentRequest.visilabsCallback!!.fail(visilabsResponse)
                            }
                        } catch (e: java.lang.Exception) {
                            e.printStackTrace()
                            Log.e(LOG_TAG, "Could not parse the response for the request : " + response.raw().request.url.toString())
                            val visilabsResponse = VisilabsResponse(null, null, rawJsonResponse, null, rawJsonResponse)
                            currentRequest.visilabsCallback!!.fail(visilabsResponse)
                        }
                    }

                    override fun onFailure(call: Call<ResponseBody?>?, t: Throwable) {
                        Log.e(LOG_TAG, "Fail Request " + t.message)
